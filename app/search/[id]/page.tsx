...
export default function Page({
  ...
  const masterData = use(fetch(`https://api.discogs.com/masters/${id}`).then(res => res.json()))
  const releaseData = use(fetch(`https://api.discogs.com/releases/${masterData.main_release}`).then(res => res.json()))
          ...
          <div className="flex-1">
            ...
            {releaseData.labels?.length > 0 && (
              <p className="text-sm text-gray-500">
                Kiadó:{' '}
                <span className="font-medium">{releaseData.labels[0].name}</span>
              </p>
            )}
            {releaseData.country && (
